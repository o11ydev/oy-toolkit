...
func NewTextFileCollector(logger log.Logger, path string) (prometheus.Collector, error) {
	c := &textFileCollector{
		path:   path,
		logger: logger,
	}
...
func (c *textFileCollector) Collect(ch chan<- prometheus.Metric) {
	...
	//if err != nil && c.path != "" {
	//	errored = true
	//	level.Error(c.logger).Log("msg", "failed to read textfile", "path", c.path, "err", err)
	//	return
	//}

	mtime, err := c.processFile(c.path, ch)
	if err != nil {
		errored = true
		level.Error(c.logger).Log("msg", "failed to collect textfile data", "file", c.path, "err", err)
		return
	}
